...
figure; imagesc(xo, yo, abs(vq)/max(vq(:))); hold on; plot(100, 200, 'or', 'MarkerSize', 8, 'LineWidth', 3); 
% plot(0, 150, 'or');
axis equal; axis([-175 175 50 500]); colorbar
ax = gca;
ax.FontSize = 24;
xlabel('x-axis (mm)','FontSize',24)
ylabel('z-axis (mm)','FontSize',24)
legend('Actual Location')

norm_m = abs(vq)/max(vq(:));
[~, ind] = max(norm_m(:));
plot(xq(ind), yq(ind), 'xk', 'MarkerSize', 12, 'LineWidth', 3)
err = sqrt((xq(ind)-100)^2 + (yq(ind)-200)^2)*1
